...
- **Viewer panel**: Tab buttons at top to switch between viewers (Web, PDF, Image, Examples, Console). Active viewer is highlighted. Each viewer has its own toolbar with an "External" button to open in a standalone application. There is no "Folder" tab — folder browsing now lives in the launcher column (see Quick File Browser Panel below) in Focus layout, or via the 📁-preview/"Add to Project" flows; `column2_mode == "folder"` still exists internally as the viewer's fallback mode.
...
- **Viewer tab buttons**: ⏱ Time, Web, PDF, Image, Examples, Console. ⏱ uses an emoji + text label ("⏱ Time", not a system icon) and only appears when `kimai_url` + `kimai_token` are configured. Others show system theme icons with text fallback.
...
- **Ordering gotcha**: this panel is built earlier in `build_main_content()`'s column-1 section than the main Folder-viewer-tab widgets (`folder_path_label`/`folder_browser`/`folder_icon_view`, built later in the "viewer panel" section). Since the panel's expanded state now persists, it can call `populate_folder_browser()` on the very first-ever build of a project that starts pre-expanded — before those main-viewer widgets exist. `populate_folder_browser()` guards all four target widgets (main + launcher-panel, tree + icons) with `getattr(self, 'x', None) is not None` for this reason, not just the launcher-panel ones.
