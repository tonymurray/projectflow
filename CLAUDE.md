...
- **Docs / Resources tabs**: filter `_build_grouped_categories()`'s two buckets down to just the active one (see Group-by-Type Launcher View for the classifier) — the search box and "+ Add" button still show above it (no "☰ Group" button in Focus layout, the tabs subsume that role).
...
A "🗂️ Group" toggle in the launcher column header (Standard layout only — Focus layout uses the Docs/Resources tabs above instead, sharing the same underlying classifier/pooling) dynamically regroups every launcher across every category into two sections — **Docs / Resources** — without ever modifying the project's category structure or JSON file. (A third bucket, Websites, existed until the Focus-layout tab bar above was added; it's now folded into Resources, sorted first within it, since there wasn't room for a dedicated Web tab alongside Files/Docs/Resources/Apps.)

- **Classifier** (`_classify_launcher_item()`): local file with a doc extension (`.md`/`.html`/`.htm`/`.pdf`/`.txt`, and not a website) → Docs; everything else → Resources. `_is_website_item()` (a `http(s)://` path or `firefox`/`chrome` app) is checked separately and used both to exclude an item from Docs and to sort it first within Resources (`_build_grouped_categories()`'s stable sort).
...
- **No drag-and-drop in this view**: items render as plain `QPushButton`s instead of `DraggableItemButton`s, because the existing drag machinery (`CategoryDropZone`) moves items between *real* categories on drop — reusing it here would have silently rewritten the config. Gated by `_is_grouped_view_active()`, which covers both entry points (legacy Standard-layout `group_by_type`, and the Focus-layout Docs/Resources tabs).
- **Manual correction**: right-click → "📁 Move display to…" (also gated by `_is_grouped_view_active()`) lets you override a misclassified item's bucket, now a Docs/Resources choice. The override is stored in `.projectflow_settings.json` under `grouping_overrides[<config path>][<item path>]` — never in the project file — and wins over the heuristic on future loads. `_get_grouping_override()` transparently maps the retired `Documentation`/`Websites` bucket names onto `Docs`/`Resources` for overrides saved before this collapse.
...
- Default Viewer: Select which viewer opens by default (notes, webview, console, pdf, image, time, help — ordered to match the viewer tab row, with help appended last since it isn't a tab)
