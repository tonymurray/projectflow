...
- `active_launcher_tab`: `"files"`/`"docs"`/`"resources"`/`"apps"` — remembers this project's last Focus-layout launcher tab (see UI Features → Launcher Tab Bar). Omitted means `"files"` (the default). Written automatically when a tab is clicked — not intended for manual editing.
