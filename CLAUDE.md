...
**Cookie/session persistence**: `self.webview`/`self.notes_webview` both set `ForcePersistentCookies` on `QWebEngineProfile.defaultProfile()` (`__init__`, before either webview is created) — but until 2026-08-25 never called `setPersistentStoragePath()`/`setCachePath()`, leaving cookie/cache storage at Qt's implicit, undocumented default location. Now pinned explicitly to `~/.local/share/ProjectFlow/webengine-profile/` (+ a `cache` subfolder), a fixed, inspectable, app-controlled directory — makes staying logged into local-network sites (e.g. a self-hosted Nextcloud) across app restarts deterministic rather than dependent on wherever the default profile happened to resolve. Investigated as part of a broader look at making the Web viewer viable for logging into local sites (see `docs/cookies_and_passwords.md`); a separate custom password-autofill feature (OS-keyring-backed, since QWebEngine has no native save-password UI) was scoped but deliberately not built yet — session-cookie persistence alone covers most of the practical need.
...
- **Path mapping checkbox**: "Path mapping" — moved here from a title-bar "⇄" icon button (was easily confused with the Focus-layout toggle when always visible), and moved again to the very bottom of the form (below Desktop Menu Entry, in its own `self._settings_path_mapping_section` container), since it's a quite obscure setting. Help text underneath: "Remap to, for example, a mounted share from a network computer. See mapping paths in the main Settings." Deferred to Save like every other field here (`_apply_settings()` reads `self._settings_path_mapping_checkbox.isChecked()` into `self.config_path_mapping`) — it used to self-toggle and call `refresh_projects()` immediately on click (`_toggle_path_mapping()`, now removed), which reloaded/rebuilt the whole app right under the user while they were still in the Settings viewer; a real usability bug caught by the user, not just a style preference. The section (checkbox + help text) is hidden as one unit via plain `QWidget.setVisible()` when no global `path_mappings` are configured — it's no longer inside `form_layout`'s `QFormLayout`, unlike the Kimai row below, so `setRowVisible()` doesn't apply here.
- **"Project Actions" section**: currently just "🔍 Scan for docs" (`_show_doc_scan_dialog()`, unchanged — still its own self-contained modal `QDialog` for picking which discovered files to add; only the entry-point button moved here from the title bar's old "🔍 Scan Docs"). Help text underneath: "Scans the default folder for .md, .html files, optionally add to documents/launchers."
- **Fields**: Project Name, Project Color (+ Clear), Layout (three-columns checkbox), Default Viewer, Default Launcher Tab, PDF File, Web URL, Image File, Console Path, Folder Start Path, Terminal, Browser Links, Kimai Project ID (row hidden via `QFormLayout.setRowVisible()` when Kimai isn't configured, rather than the old dialog's "don't create the row at all" approach — the widget must always exist now that the form is permanent), a "Project Actions" section (Scan for docs, with help text), a "Desktop Menu Entry" section (Create Menu Entry button, unchanged), and finally Path Mapping (checkbox + help text, hidden unless configured) at the very bottom.
